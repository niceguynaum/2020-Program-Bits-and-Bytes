...
class Noise : public stk::Generator
{
public:
...
inline stk::StkFloat Noise::tick(void)
{
	return lastFrame_[0] = (stk::StkFloat)(2.0 * rand() / (RAND_MAX + 1.0) - 1.0);
}

inline stk::StkFrames& Noise::tick(stk::StkFrames& frames, unsigned int channel)
{
	...
	for (unsigned int i = 0; i < frames.frames(); i++, samples += hop)
		*samples = (stk::StkFloat) (2.0 * rand() / (RAND_MAX + 1.0) - 1.0);
